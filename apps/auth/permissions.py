...
def _get_superuser_role_str() -> str:
    """Dynamically resolves the superuser/admin role enum member safely."""
    if hasattr(UserRole, "SUPERUSER"):
        return _extract_role_str(UserRole.SUPERUSER)
    if hasattr(UserRole, "SUPERVISOR"):
        return _extract_role_str(UserRole.SUPERVISOR)
    if hasattr(UserRole, "ADMIN"):
        return _extract_role_str(UserRole.ADMIN)
    return "superuser"
...
class IsSuperUser(BasePermission):
    ...
    async def has_permission(
        ...
        user_role = _extract_role_str(getattr(user, "role", None))
        superuser_role = _get_superuser_role_str()

        return user_role == superuser_role or user_role in ["superuser", "supervisor", "admin"]
...
class IsUser(BasePermission):
    ...
    async def has_permission(
        ...
        target_role = ""
        if hasattr(UserRole, "USER"):
            target_role = _extract_role_str(UserRole.USER)
        else:
            target_role = "user"

        return user_role == target_role or user_role in ["user", "student", "client"]
...
class HasRole(BasePermission):
    ...
    async def has_permission(
        ...
        return _extract_role_str(getattr(user, "role", None)) in self.roles
...
class IsSuperUserOrRole(BasePermission):
    ...
    async def has_permission(
        ...
        if user_role == superuser_role or user_role in ["superuser", "supervisor", "admin"]:
            return True
